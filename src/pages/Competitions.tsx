          ...
          <div className="lg:col-span-2 space-y-8">
            <section className="bg-white rounded-3xl p-10 border border-beer-brown/5 shadow-sm">
              <h2 className="text-3xl mb-8 flex items-center gap-3">
                <Trophy className="text-beer-amber h-8 w-8" />
                2026 Competition Schedule
              </h2>
              <div className="overflow-hidden rounded-xl border border-beer-brown/10">
                <table className="w-full text-left">
                  <thead className="bg-beer-brown text-beer-cream uppercase text-xs font-bold tracking-widest">
                    <tr>
                      <th className="px-6 py-4">Month</th>
                      <th className="px-6 py-4">Style / Category</th>
                      <th className="px-6 py-4">Format</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-beer-brown/10">
                    {competitionSchedule.map((comp, index) => (
                      <tr key={index} className="hover:bg-beer-amber/5 transition-colors">
                        <td className="px-6 py-4 font-bold">{comp.month}</td>
                        <td className="px-6 py-4 text-beer-brown/80">{comp.style}</td>
                        <td className="px-6 py-4">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                            comp.format === 'BJCP Judged' ? 'bg-beer-brown text-beer-cream' : 'bg-beer-amber text-beer-brown'
                          }`}>
                            {comp.format}
                          </span>
                        </td>
          ...
          <div className="space-y-8">
            <div className="bg-beer-brown text-beer-cream rounded-3xl p-8 shadow-xl relative overflow-hidden">
              <div className="relative z-10">
                <h3 className="text-2xl text-beer-amber mb-6">How to Enter</h3>
                <ul className="space-y-4 text-beer-cream/80 mb-8">
                  <li className="flex gap-3">
                    <span className="h-6 w-6 rounded-full bg-beer-amber text-beer-brown flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">1</span>
                    Check the monthly style requirement.
                  </li>
                  <li className="flex gap-3">
                    <span className="h-6 w-6 rounded-full bg-beer-amber text-beer-brown flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">2</span>
                    Bring two 12oz bottles (for BJCP) or a growler/keg (for Popular Vote).
                  </li>
                  <li className="flex gap-3">
                    <span className="h-6 w-6 rounded-full bg-beer-amber text-beer-brown flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">3</span>
                    Register your entry at the meeting check-in desk.
                  </li>
                </ul>
                <button className="w-full py-4 bg-beer-amber text-beer-brown rounded-xl font-bold hover:bg-opacity-90 transition">
                  Full Rulebook
                </button>
              </div>
            </div>
